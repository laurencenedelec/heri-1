...
ibs=matrix(rep(0,nrow(testfrm)*nrow(testfrm)),ncol=nrow(testfrm))
ibs[,][g]<- ibsd(testfrm[i,14+g],testfrm[j,14+g])
ibsd=function(x,y) {
if (c(x,y)=x
return(p)}
...
##rlist<- foreach(j = 14:(ncol(testfrm)-pas),.combine='rbind') %dopar% pcov-vvdim(testfrm[,pheno],testfrm[,j:(j+pas)]) 
##pcvpas[1:(ncol(testfrm)-pas-13),ipheno] <- rlist[1:(ncol(testfrm)-pas-13),1]

 mibs=read.table('~/NFG/ler/plink.mibs', header=F , sep='')
 dmibs<-1-mibs
 out<- dcor_vm(testfrm[,pheno],dmibs[-c(1737,4756), -c(1737,4756)],1)
##}
