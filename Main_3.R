...
param.list[[4]] <- list(N = rep(20, times = N_Estimation),
                        N_SNPS = seq(from = 20, to = 1000, by = 1000/N_Estimation),
                        N_real_coeff = rep(20, times = N_Estimation),
                        b = c(2,2),
                        variable = "N_SNPS")

param.list[[5]] <- list(N = rep(1000, times = N_Estimation),
                        N_SNPS = rep(1000, times = N_Estimation),
                        N_real_coeff = seq(from = 20, to = 1000, by = 1000/N_Estimation),
                        b = c(2,2),
                        variable = "N_real_coeff")

param.list[[6]] <- list(N = seq(from = 20, to = 1000, by = 1000/N_Estimation),
                        N_SNPS = rep(20, times = N_Estimation),
                        N_real_coeff = rep(20, times = N_Estimation),
                        b = c(2,2),
                        variable = "N_real_coeff")

param.list[[7]] <- list(N = seq(from = 20, to = 1000, by = 1000/N_Estimation),
                        N_SNPS = seq(from = 20, to = 1000, by = 1000/N_Estimation),
                        N_real_coeff = rep(20, times = N_Estimation),
                        b = c(2,2),
                        variable = c("N", "N_SNPS"))
