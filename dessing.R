...
pdf(file= paste("~/NFG/result/toutH", names(pvalue)[b],"-ch",chr,".pdf",sep=""))
#par(mfrow=c(3,2), mar=c(1.1,4.1,3.5,2.1))
if (chr==4) {par(mfrow=c(10,2))} else {par(mfrow=c(9,2))}
plot(pvalue[,2]/1000000,-log(pvalue[, b],10) ,col="red",pch=18,xlim=c(40,90),ylim=c(0,10),ann=FALSE)
sel5=read.table(paste("~/NFG/resultvv/allessaiHdap",chr,"pR","repli=",Repli,"pas=",5,"select",sep=""))
points(sel5[,3]/1000000,-log(sel5[,8],10) ,col="blue",pch=19)

if (!chr==4) {sel10=read.table(paste("~/NFG/resultvv/allessaiHdap",chr,"pR","repli=",Repli,"pas=",10,"select",sep=""))
...
if (chr==4) {dcor=read.table(paste("~/NFG/result/allessaiHdap",chr,"dr",15,sep=""),header=T,sep=" ")
##pdf(file= paste("~/NFG/result/allH",names(dcor)[b],"-ch",chr,"dc",pas,".pdf",sep=""))
plot(dcor[,2]/1000000,dcor[, b] ,col="red",pch=18,xlim=c(40,90),ylim=c(0,.1),ann=FALSE)
title(xlab=paste("Chr=",chr," pas=",10," pheno=",names(dcor)[b]," position (MB) dcor",sep=""), col.lab=rgb(0,0.5,0))
title(ylab="dcor pas 10",col.lab=rgb(.5,0,0))}
...
##pdf(file= paste("~/NFG/result/allH",names(dcor)[b],"-ch",chr,"dc",pas,".pdf",sep=""))
plot(dcor[,2]/1000000,-log(dcor[, b],10) ,col="red",pch=18,xlim=c(40,90),ylim=c(0,.1),ann=FALSE)
...
##pdf(file= paste("~/NFG/result/allH",names(dcor)[b],"-ch",chr,"dc",pas,".pdf",sep=""))
plot(dcor[,2]/1000000,-log(dcor[, b],10) ,col="red",pch=18,xlim=c(40,90),ylim=c(0,.1),ann=FALSE)
title(xlab=paste("Chr=",chr," pas=20"," pheno=",names(dcor)[b]," position (MB) pmoi pas 10",sep=""), col.lab=rgb(0,0.5,0))
title(ylab="pttest",col.lab=rgb(.5,0,0))
