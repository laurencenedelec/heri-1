...
rlist<- foreach(j = 14:(ncol(testfrm)-pas),.combine='rbind') %dopar% dcor_vvdim(testfrm[,pheno],(testfrm[,j:(j+pas)])) 
pcvpas[1:(ncol(testfrm)-pas-13),ipheno] <- rlist[1:(ncol(testfrm)-pas-13),1]

## mibs=read.table('~/NFG/ler/plink.mibs', header=F , sep='')
## dmibs<-1-mibs
## out<- dcor-vm(testfrm[,pheno],dmibs[-c(1737,4756), -c(1737,4756)],1)
